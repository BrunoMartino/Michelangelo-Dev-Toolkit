...
Write a working file (e.g. `/tmp/sistina-arch.svg`) containing **one** `<svg>...</svg>`. Facts come only from Steps 0–0b.
...
```bash
sh .claude/skills/sistina-arch/scripts/check.sh /tmp/sistina-arch.svg
```
...
```bash
sh .claude/skills/sistina-arch/scripts/inject.sh \
  --svg /tmp/sistina-arch.svg \
  --title "…" \
