...
description: >-
  ...
  string and exposed query port, and registers the server in the project-scope .mcp.json.
...
## Step 4 — Register in `.mcp.json` (project root)
...
The connection string is a **secret**: warn the user that `.mcp.json` will contain it and must stay out of version control (check `.gitignore`), or use the `${VAR}` env-expansion form supported by Claude Code if they prefer. Never write it to `.env` (only `.env.example` placeholders, per project rules).

## Step 5 — Verify & report

Ask the user to restart the Claude Code session (or run `claude mcp list` to confirm registration), then call a cheap tool (e.g. list tables/schemas) to confirm connectivity. Report: repo location, install steps executed, server name registered, env vars set (names only, never the values), verification result.
