...
Follow always-apply project rules (including harness gates under `.cursor/rules/`). On a Nest target, also follow `nest-conventions`.
...
- End with a short summary: graph action taken (install/incremental/force/cluster-only/AST-only), `graphify-out/` files touched, graph mode + commit/hash, harness templates overwritten, feature files created/updated under `docs/harness/features/` and the ones **pending** user answers to the 4 questions, manual commands for anything skipped or failed.
